...
comment on column ATTACHMENT.relationid is '关联id';
comment on column ATTACHMENT.type is '0-用户头像';
...
-- Add comments to the table 
comment on table USERS is '注册会员用户表';
-- Add comments to the columns 
comment on column USERS.nickname is '昵称';
comment on column USERS.password is '密码';
comment on column USERS.fullname is '全称';
comment on column USERS.email is '邮箱';
comment on column USERS.mobile is '手机';
comment on column USERS.fromcity is '城市';
comment on column USERS.sign is '签名';
comment on column USERS.gender is '性别 0-男，1-女';
comment on column USERS.validateemail is '邮箱是否认证 0-否 1-是';
comment on column USERS.validateuser is '用户是否认证 0-否 1-是';
comment on column USERS.starts is '可用积分';
comment on column USERS.validateinfo is '认证信息';
comment on column USERS.createdate is '创建时间';
comment on column USERS.iseffective is '是否有效用户0-有效，1-无效';
-- Create/Recreate primary, unique and foreign key constraints 
alter table USERS
